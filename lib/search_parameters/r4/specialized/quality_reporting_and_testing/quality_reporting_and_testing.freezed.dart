// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies

part of 'quality_reporting_and_testing.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

/// @nodoc
class _$MeasureSearchTearOff {
  const _$MeasureSearchTearOff();

// ignore: unused_element
  _MeasureSearch call(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<String> composed_of,
      List<SearchToken> context,
      List<String> context_quantity,
      List<SearchToken> context_type,
      List<SearchDate> date,
      List<String> depends_on,
      List<String> derived_from,
      List<SearchString> description,
      List<SearchDate> effective,
      List<SearchToken> identifier,
      List<SearchToken> jurisdiction,
      List<SearchString> name,
      List<String> predecessor,
      List<SearchString> publisher,
      List<SearchToken> status,
      List<String> successor,
      List<SearchString> title,
      List<SearchToken> topic,
      List<SearchUri> url,
      List<SearchToken> version,
      List<String> context_type_quantity,
      List<String> context_type_value}) {
    return _MeasureSearch(
      searchId: searchId,
      searchLastUpdated: searchLastUpdated,
      composed_of: composed_of,
      context: context,
      context_quantity: context_quantity,
      context_type: context_type,
      date: date,
      depends_on: depends_on,
      derived_from: derived_from,
      description: description,
      effective: effective,
      identifier: identifier,
      jurisdiction: jurisdiction,
      name: name,
      predecessor: predecessor,
      publisher: publisher,
      status: status,
      successor: successor,
      title: title,
      topic: topic,
      url: url,
      version: version,
      context_type_quantity: context_type_quantity,
      context_type_value: context_type_value,
    );
  }
}

/// @nodoc
// ignore: unused_element
const $MeasureSearch = _$MeasureSearchTearOff();

/// @nodoc
mixin _$MeasureSearch {
  List<Id> get searchId;
  List<SearchDate> get searchLastUpdated; //List<SearchToken> searchTag,
//List<SearchUri> searchProfile,
//List<SearchToken> searchSecurity,
//List<String> searchText,
//List<String> searchContent,
//List<SearchString> searchList,
// List<SearchString> searchHas,
//List<SearchToken> searchType,
  List<String> get composed_of;
  List<SearchToken> get context;
  List<String> get context_quantity;
  List<SearchToken> get context_type;
  List<SearchDate> get date;
  List<String> get depends_on;
  List<String> get derived_from;
  List<SearchString> get description;
  List<SearchDate> get effective;
  List<SearchToken> get identifier;
  List<SearchToken> get jurisdiction;
  List<SearchString> get name;
  List<String> get predecessor;
  List<SearchString> get publisher;
  List<SearchToken> get status;
  List<String> get successor;
  List<SearchString> get title;
  List<SearchToken> get topic;
  List<SearchUri> get url;
  List<SearchToken> get version;
  List<String> get context_type_quantity;
  List<String> get context_type_value;

  $MeasureSearchCopyWith<MeasureSearch> get copyWith;
}

/// @nodoc
abstract class $MeasureSearchCopyWith<$Res> {
  factory $MeasureSearchCopyWith(
          MeasureSearch value, $Res Function(MeasureSearch) then) =
      _$MeasureSearchCopyWithImpl<$Res>;
  $Res call(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<String> composed_of,
      List<SearchToken> context,
      List<String> context_quantity,
      List<SearchToken> context_type,
      List<SearchDate> date,
      List<String> depends_on,
      List<String> derived_from,
      List<SearchString> description,
      List<SearchDate> effective,
      List<SearchToken> identifier,
      List<SearchToken> jurisdiction,
      List<SearchString> name,
      List<String> predecessor,
      List<SearchString> publisher,
      List<SearchToken> status,
      List<String> successor,
      List<SearchString> title,
      List<SearchToken> topic,
      List<SearchUri> url,
      List<SearchToken> version,
      List<String> context_type_quantity,
      List<String> context_type_value});
}

/// @nodoc
class _$MeasureSearchCopyWithImpl<$Res>
    implements $MeasureSearchCopyWith<$Res> {
  _$MeasureSearchCopyWithImpl(this._value, this._then);

  final MeasureSearch _value;
  // ignore: unused_field
  final $Res Function(MeasureSearch) _then;

  @override
  $Res call({
    Object searchId = freezed,
    Object searchLastUpdated = freezed,
    Object composed_of = freezed,
    Object context = freezed,
    Object context_quantity = freezed,
    Object context_type = freezed,
    Object date = freezed,
    Object depends_on = freezed,
    Object derived_from = freezed,
    Object description = freezed,
    Object effective = freezed,
    Object identifier = freezed,
    Object jurisdiction = freezed,
    Object name = freezed,
    Object predecessor = freezed,
    Object publisher = freezed,
    Object status = freezed,
    Object successor = freezed,
    Object title = freezed,
    Object topic = freezed,
    Object url = freezed,
    Object version = freezed,
    Object context_type_quantity = freezed,
    Object context_type_value = freezed,
  }) {
    return _then(_value.copyWith(
      searchId: searchId == freezed ? _value.searchId : searchId as List<Id>,
      searchLastUpdated: searchLastUpdated == freezed
          ? _value.searchLastUpdated
          : searchLastUpdated as List<SearchDate>,
      composed_of: composed_of == freezed
          ? _value.composed_of
          : composed_of as List<String>,
      context:
          context == freezed ? _value.context : context as List<SearchToken>,
      context_quantity: context_quantity == freezed
          ? _value.context_quantity
          : context_quantity as List<String>,
      context_type: context_type == freezed
          ? _value.context_type
          : context_type as List<SearchToken>,
      date: date == freezed ? _value.date : date as List<SearchDate>,
      depends_on: depends_on == freezed
          ? _value.depends_on
          : depends_on as List<String>,
      derived_from: derived_from == freezed
          ? _value.derived_from
          : derived_from as List<String>,
      description: description == freezed
          ? _value.description
          : description as List<SearchString>,
      effective: effective == freezed
          ? _value.effective
          : effective as List<SearchDate>,
      identifier: identifier == freezed
          ? _value.identifier
          : identifier as List<SearchToken>,
      jurisdiction: jurisdiction == freezed
          ? _value.jurisdiction
          : jurisdiction as List<SearchToken>,
      name: name == freezed ? _value.name : name as List<SearchString>,
      predecessor: predecessor == freezed
          ? _value.predecessor
          : predecessor as List<String>,
      publisher: publisher == freezed
          ? _value.publisher
          : publisher as List<SearchString>,
      status: status == freezed ? _value.status : status as List<SearchToken>,
      successor:
          successor == freezed ? _value.successor : successor as List<String>,
      title: title == freezed ? _value.title : title as List<SearchString>,
      topic: topic == freezed ? _value.topic : topic as List<SearchToken>,
      url: url == freezed ? _value.url : url as List<SearchUri>,
      version:
          version == freezed ? _value.version : version as List<SearchToken>,
      context_type_quantity: context_type_quantity == freezed
          ? _value.context_type_quantity
          : context_type_quantity as List<String>,
      context_type_value: context_type_value == freezed
          ? _value.context_type_value
          : context_type_value as List<String>,
    ));
  }
}

/// @nodoc
abstract class _$MeasureSearchCopyWith<$Res>
    implements $MeasureSearchCopyWith<$Res> {
  factory _$MeasureSearchCopyWith(
          _MeasureSearch value, $Res Function(_MeasureSearch) then) =
      __$MeasureSearchCopyWithImpl<$Res>;
  @override
  $Res call(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<String> composed_of,
      List<SearchToken> context,
      List<String> context_quantity,
      List<SearchToken> context_type,
      List<SearchDate> date,
      List<String> depends_on,
      List<String> derived_from,
      List<SearchString> description,
      List<SearchDate> effective,
      List<SearchToken> identifier,
      List<SearchToken> jurisdiction,
      List<SearchString> name,
      List<String> predecessor,
      List<SearchString> publisher,
      List<SearchToken> status,
      List<String> successor,
      List<SearchString> title,
      List<SearchToken> topic,
      List<SearchUri> url,
      List<SearchToken> version,
      List<String> context_type_quantity,
      List<String> context_type_value});
}

/// @nodoc
class __$MeasureSearchCopyWithImpl<$Res>
    extends _$MeasureSearchCopyWithImpl<$Res>
    implements _$MeasureSearchCopyWith<$Res> {
  __$MeasureSearchCopyWithImpl(
      _MeasureSearch _value, $Res Function(_MeasureSearch) _then)
      : super(_value, (v) => _then(v as _MeasureSearch));

  @override
  _MeasureSearch get _value => super._value as _MeasureSearch;

  @override
  $Res call({
    Object searchId = freezed,
    Object searchLastUpdated = freezed,
    Object composed_of = freezed,
    Object context = freezed,
    Object context_quantity = freezed,
    Object context_type = freezed,
    Object date = freezed,
    Object depends_on = freezed,
    Object derived_from = freezed,
    Object description = freezed,
    Object effective = freezed,
    Object identifier = freezed,
    Object jurisdiction = freezed,
    Object name = freezed,
    Object predecessor = freezed,
    Object publisher = freezed,
    Object status = freezed,
    Object successor = freezed,
    Object title = freezed,
    Object topic = freezed,
    Object url = freezed,
    Object version = freezed,
    Object context_type_quantity = freezed,
    Object context_type_value = freezed,
  }) {
    return _then(_MeasureSearch(
      searchId: searchId == freezed ? _value.searchId : searchId as List<Id>,
      searchLastUpdated: searchLastUpdated == freezed
          ? _value.searchLastUpdated
          : searchLastUpdated as List<SearchDate>,
      composed_of: composed_of == freezed
          ? _value.composed_of
          : composed_of as List<String>,
      context:
          context == freezed ? _value.context : context as List<SearchToken>,
      context_quantity: context_quantity == freezed
          ? _value.context_quantity
          : context_quantity as List<String>,
      context_type: context_type == freezed
          ? _value.context_type
          : context_type as List<SearchToken>,
      date: date == freezed ? _value.date : date as List<SearchDate>,
      depends_on: depends_on == freezed
          ? _value.depends_on
          : depends_on as List<String>,
      derived_from: derived_from == freezed
          ? _value.derived_from
          : derived_from as List<String>,
      description: description == freezed
          ? _value.description
          : description as List<SearchString>,
      effective: effective == freezed
          ? _value.effective
          : effective as List<SearchDate>,
      identifier: identifier == freezed
          ? _value.identifier
          : identifier as List<SearchToken>,
      jurisdiction: jurisdiction == freezed
          ? _value.jurisdiction
          : jurisdiction as List<SearchToken>,
      name: name == freezed ? _value.name : name as List<SearchString>,
      predecessor: predecessor == freezed
          ? _value.predecessor
          : predecessor as List<String>,
      publisher: publisher == freezed
          ? _value.publisher
          : publisher as List<SearchString>,
      status: status == freezed ? _value.status : status as List<SearchToken>,
      successor:
          successor == freezed ? _value.successor : successor as List<String>,
      title: title == freezed ? _value.title : title as List<SearchString>,
      topic: topic == freezed ? _value.topic : topic as List<SearchToken>,
      url: url == freezed ? _value.url : url as List<SearchUri>,
      version:
          version == freezed ? _value.version : version as List<SearchToken>,
      context_type_quantity: context_type_quantity == freezed
          ? _value.context_type_quantity
          : context_type_quantity as List<String>,
      context_type_value: context_type_value == freezed
          ? _value.context_type_value
          : context_type_value as List<String>,
    ));
  }
}

/// @nodoc
class _$_MeasureSearch extends _MeasureSearch {
  _$_MeasureSearch(
      {this.searchId,
      this.searchLastUpdated,
      this.composed_of,
      this.context,
      this.context_quantity,
      this.context_type,
      this.date,
      this.depends_on,
      this.derived_from,
      this.description,
      this.effective,
      this.identifier,
      this.jurisdiction,
      this.name,
      this.predecessor,
      this.publisher,
      this.status,
      this.successor,
      this.title,
      this.topic,
      this.url,
      this.version,
      this.context_type_quantity,
      this.context_type_value})
      : super._();

  @override
  final List<Id> searchId;
  @override
  final List<SearchDate> searchLastUpdated;
  @override //List<SearchToken> searchTag,
//List<SearchUri> searchProfile,
//List<SearchToken> searchSecurity,
//List<String> searchText,
//List<String> searchContent,
//List<SearchString> searchList,
// List<SearchString> searchHas,
//List<SearchToken> searchType,
  final List<String> composed_of;
  @override
  final List<SearchToken> context;
  @override
  final List<String> context_quantity;
  @override
  final List<SearchToken> context_type;
  @override
  final List<SearchDate> date;
  @override
  final List<String> depends_on;
  @override
  final List<String> derived_from;
  @override
  final List<SearchString> description;
  @override
  final List<SearchDate> effective;
  @override
  final List<SearchToken> identifier;
  @override
  final List<SearchToken> jurisdiction;
  @override
  final List<SearchString> name;
  @override
  final List<String> predecessor;
  @override
  final List<SearchString> publisher;
  @override
  final List<SearchToken> status;
  @override
  final List<String> successor;
  @override
  final List<SearchString> title;
  @override
  final List<SearchToken> topic;
  @override
  final List<SearchUri> url;
  @override
  final List<SearchToken> version;
  @override
  final List<String> context_type_quantity;
  @override
  final List<String> context_type_value;

  @override
  String toString() {
    return 'MeasureSearch(searchId: $searchId, searchLastUpdated: $searchLastUpdated, composed_of: $composed_of, context: $context, context_quantity: $context_quantity, context_type: $context_type, date: $date, depends_on: $depends_on, derived_from: $derived_from, description: $description, effective: $effective, identifier: $identifier, jurisdiction: $jurisdiction, name: $name, predecessor: $predecessor, publisher: $publisher, status: $status, successor: $successor, title: $title, topic: $topic, url: $url, version: $version, context_type_quantity: $context_type_quantity, context_type_value: $context_type_value)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other is _MeasureSearch &&
            (identical(other.searchId, searchId) ||
                const DeepCollectionEquality()
                    .equals(other.searchId, searchId)) &&
            (identical(other.searchLastUpdated, searchLastUpdated) ||
                const DeepCollectionEquality()
                    .equals(other.searchLastUpdated, searchLastUpdated)) &&
            (identical(other.composed_of, composed_of) ||
                const DeepCollectionEquality()
                    .equals(other.composed_of, composed_of)) &&
            (identical(other.context, context) ||
                const DeepCollectionEquality()
                    .equals(other.context, context)) &&
            (identical(other.context_quantity, context_quantity) ||
                const DeepCollectionEquality()
                    .equals(other.context_quantity, context_quantity)) &&
            (identical(other.context_type, context_type) ||
                const DeepCollectionEquality()
                    .equals(other.context_type, context_type)) &&
            (identical(other.date, date) ||
                const DeepCollectionEquality().equals(other.date, date)) &&
            (identical(other.depends_on, depends_on) ||
                const DeepCollectionEquality()
                    .equals(other.depends_on, depends_on)) &&
            (identical(other.derived_from, derived_from) ||
                const DeepCollectionEquality()
                    .equals(other.derived_from, derived_from)) &&
            (identical(other.description, description) ||
                const DeepCollectionEquality()
                    .equals(other.description, description)) &&
            (identical(other.effective, effective) ||
                const DeepCollectionEquality()
                    .equals(other.effective, effective)) &&
            (identical(other.identifier, identifier) ||
                const DeepCollectionEquality()
                    .equals(other.identifier, identifier)) &&
            (identical(other.jurisdiction, jurisdiction) ||
                const DeepCollectionEquality()
                    .equals(other.jurisdiction, jurisdiction)) &&
            (identical(other.name, name) ||
                const DeepCollectionEquality().equals(other.name, name)) &&
            (identical(other.predecessor, predecessor) ||
                const DeepCollectionEquality()
                    .equals(other.predecessor, predecessor)) &&
            (identical(other.publisher, publisher) ||
                const DeepCollectionEquality()
                    .equals(other.publisher, publisher)) &&
            (identical(other.status, status) ||
                const DeepCollectionEquality().equals(other.status, status)) &&
            (identical(other.successor, successor) ||
                const DeepCollectionEquality()
                    .equals(other.successor, successor)) &&
            (identical(other.title, title) ||
                const DeepCollectionEquality().equals(other.title, title)) &&
            (identical(other.topic, topic) ||
                const DeepCollectionEquality().equals(other.topic, topic)) &&
            (identical(other.url, url) ||
                const DeepCollectionEquality().equals(other.url, url)) &&
            (identical(other.version, version) ||
                const DeepCollectionEquality()
                    .equals(other.version, version)) &&
            (identical(other.context_type_quantity, context_type_quantity) ||
                const DeepCollectionEquality().equals(
                    other.context_type_quantity, context_type_quantity)) &&
            (identical(other.context_type_value, context_type_value) ||
                const DeepCollectionEquality()
                    .equals(other.context_type_value, context_type_value)));
  }

  @override
  int get hashCode =>
      runtimeType.hashCode ^
      const DeepCollectionEquality().hash(searchId) ^
      const DeepCollectionEquality().hash(searchLastUpdated) ^
      const DeepCollectionEquality().hash(composed_of) ^
      const DeepCollectionEquality().hash(context) ^
      const DeepCollectionEquality().hash(context_quantity) ^
      const DeepCollectionEquality().hash(context_type) ^
      const DeepCollectionEquality().hash(date) ^
      const DeepCollectionEquality().hash(depends_on) ^
      const DeepCollectionEquality().hash(derived_from) ^
      const DeepCollectionEquality().hash(description) ^
      const DeepCollectionEquality().hash(effective) ^
      const DeepCollectionEquality().hash(identifier) ^
      const DeepCollectionEquality().hash(jurisdiction) ^
      const DeepCollectionEquality().hash(name) ^
      const DeepCollectionEquality().hash(predecessor) ^
      const DeepCollectionEquality().hash(publisher) ^
      const DeepCollectionEquality().hash(status) ^
      const DeepCollectionEquality().hash(successor) ^
      const DeepCollectionEquality().hash(title) ^
      const DeepCollectionEquality().hash(topic) ^
      const DeepCollectionEquality().hash(url) ^
      const DeepCollectionEquality().hash(version) ^
      const DeepCollectionEquality().hash(context_type_quantity) ^
      const DeepCollectionEquality().hash(context_type_value);

  @override
  _$MeasureSearchCopyWith<_MeasureSearch> get copyWith =>
      __$MeasureSearchCopyWithImpl<_MeasureSearch>(this, _$identity);
}

abstract class _MeasureSearch extends MeasureSearch {
  _MeasureSearch._() : super._();
  factory _MeasureSearch(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<String> composed_of,
      List<SearchToken> context,
      List<String> context_quantity,
      List<SearchToken> context_type,
      List<SearchDate> date,
      List<String> depends_on,
      List<String> derived_from,
      List<SearchString> description,
      List<SearchDate> effective,
      List<SearchToken> identifier,
      List<SearchToken> jurisdiction,
      List<SearchString> name,
      List<String> predecessor,
      List<SearchString> publisher,
      List<SearchToken> status,
      List<String> successor,
      List<SearchString> title,
      List<SearchToken> topic,
      List<SearchUri> url,
      List<SearchToken> version,
      List<String> context_type_quantity,
      List<String> context_type_value}) = _$_MeasureSearch;

  @override
  List<Id> get searchId;
  @override
  List<SearchDate> get searchLastUpdated;
  @override //List<SearchToken> searchTag,
//List<SearchUri> searchProfile,
//List<SearchToken> searchSecurity,
//List<String> searchText,
//List<String> searchContent,
//List<SearchString> searchList,
// List<SearchString> searchHas,
//List<SearchToken> searchType,
  List<String> get composed_of;
  @override
  List<SearchToken> get context;
  @override
  List<String> get context_quantity;
  @override
  List<SearchToken> get context_type;
  @override
  List<SearchDate> get date;
  @override
  List<String> get depends_on;
  @override
  List<String> get derived_from;
  @override
  List<SearchString> get description;
  @override
  List<SearchDate> get effective;
  @override
  List<SearchToken> get identifier;
  @override
  List<SearchToken> get jurisdiction;
  @override
  List<SearchString> get name;
  @override
  List<String> get predecessor;
  @override
  List<SearchString> get publisher;
  @override
  List<SearchToken> get status;
  @override
  List<String> get successor;
  @override
  List<SearchString> get title;
  @override
  List<SearchToken> get topic;
  @override
  List<SearchUri> get url;
  @override
  List<SearchToken> get version;
  @override
  List<String> get context_type_quantity;
  @override
  List<String> get context_type_value;
  @override
  _$MeasureSearchCopyWith<_MeasureSearch> get copyWith;
}

/// @nodoc
class _$MeasureReportSearchTearOff {
  const _$MeasureReportSearchTearOff();

// ignore: unused_element
  _MeasureReportSearch call(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<SearchDate> date,
      List<String> evaluated_resource,
      List<SearchToken> identifier,
      List<String> measure,
      List<String> patient,
      List<SearchDate> period,
      List<String> reporter,
      List<SearchToken> status,
      List<String> subject}) {
    return _MeasureReportSearch(
      searchId: searchId,
      searchLastUpdated: searchLastUpdated,
      date: date,
      evaluated_resource: evaluated_resource,
      identifier: identifier,
      measure: measure,
      patient: patient,
      period: period,
      reporter: reporter,
      status: status,
      subject: subject,
    );
  }
}

/// @nodoc
// ignore: unused_element
const $MeasureReportSearch = _$MeasureReportSearchTearOff();

/// @nodoc
mixin _$MeasureReportSearch {
  List<Id> get searchId;
  List<SearchDate> get searchLastUpdated; //List<SearchToken> searchTag,
//List<SearchUri> searchProfile,
//List<SearchToken> searchSecurity,
//List<String> searchText,
//List<String> searchContent,
//List<SearchString> searchList,
// List<SearchString> searchHas,
//List<SearchToken> searchType,
  List<SearchDate> get date;
  List<String> get evaluated_resource;
  List<SearchToken> get identifier;
  List<String> get measure;
  List<String> get patient;
  List<SearchDate> get period;
  List<String> get reporter;
  List<SearchToken> get status;
  List<String> get subject;

  $MeasureReportSearchCopyWith<MeasureReportSearch> get copyWith;
}

/// @nodoc
abstract class $MeasureReportSearchCopyWith<$Res> {
  factory $MeasureReportSearchCopyWith(
          MeasureReportSearch value, $Res Function(MeasureReportSearch) then) =
      _$MeasureReportSearchCopyWithImpl<$Res>;
  $Res call(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<SearchDate> date,
      List<String> evaluated_resource,
      List<SearchToken> identifier,
      List<String> measure,
      List<String> patient,
      List<SearchDate> period,
      List<String> reporter,
      List<SearchToken> status,
      List<String> subject});
}

/// @nodoc
class _$MeasureReportSearchCopyWithImpl<$Res>
    implements $MeasureReportSearchCopyWith<$Res> {
  _$MeasureReportSearchCopyWithImpl(this._value, this._then);

  final MeasureReportSearch _value;
  // ignore: unused_field
  final $Res Function(MeasureReportSearch) _then;

  @override
  $Res call({
    Object searchId = freezed,
    Object searchLastUpdated = freezed,
    Object date = freezed,
    Object evaluated_resource = freezed,
    Object identifier = freezed,
    Object measure = freezed,
    Object patient = freezed,
    Object period = freezed,
    Object reporter = freezed,
    Object status = freezed,
    Object subject = freezed,
  }) {
    return _then(_value.copyWith(
      searchId: searchId == freezed ? _value.searchId : searchId as List<Id>,
      searchLastUpdated: searchLastUpdated == freezed
          ? _value.searchLastUpdated
          : searchLastUpdated as List<SearchDate>,
      date: date == freezed ? _value.date : date as List<SearchDate>,
      evaluated_resource: evaluated_resource == freezed
          ? _value.evaluated_resource
          : evaluated_resource as List<String>,
      identifier: identifier == freezed
          ? _value.identifier
          : identifier as List<SearchToken>,
      measure: measure == freezed ? _value.measure : measure as List<String>,
      patient: patient == freezed ? _value.patient : patient as List<String>,
      period: period == freezed ? _value.period : period as List<SearchDate>,
      reporter:
          reporter == freezed ? _value.reporter : reporter as List<String>,
      status: status == freezed ? _value.status : status as List<SearchToken>,
      subject: subject == freezed ? _value.subject : subject as List<String>,
    ));
  }
}

/// @nodoc
abstract class _$MeasureReportSearchCopyWith<$Res>
    implements $MeasureReportSearchCopyWith<$Res> {
  factory _$MeasureReportSearchCopyWith(_MeasureReportSearch value,
          $Res Function(_MeasureReportSearch) then) =
      __$MeasureReportSearchCopyWithImpl<$Res>;
  @override
  $Res call(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<SearchDate> date,
      List<String> evaluated_resource,
      List<SearchToken> identifier,
      List<String> measure,
      List<String> patient,
      List<SearchDate> period,
      List<String> reporter,
      List<SearchToken> status,
      List<String> subject});
}

/// @nodoc
class __$MeasureReportSearchCopyWithImpl<$Res>
    extends _$MeasureReportSearchCopyWithImpl<$Res>
    implements _$MeasureReportSearchCopyWith<$Res> {
  __$MeasureReportSearchCopyWithImpl(
      _MeasureReportSearch _value, $Res Function(_MeasureReportSearch) _then)
      : super(_value, (v) => _then(v as _MeasureReportSearch));

  @override
  _MeasureReportSearch get _value => super._value as _MeasureReportSearch;

  @override
  $Res call({
    Object searchId = freezed,
    Object searchLastUpdated = freezed,
    Object date = freezed,
    Object evaluated_resource = freezed,
    Object identifier = freezed,
    Object measure = freezed,
    Object patient = freezed,
    Object period = freezed,
    Object reporter = freezed,
    Object status = freezed,
    Object subject = freezed,
  }) {
    return _then(_MeasureReportSearch(
      searchId: searchId == freezed ? _value.searchId : searchId as List<Id>,
      searchLastUpdated: searchLastUpdated == freezed
          ? _value.searchLastUpdated
          : searchLastUpdated as List<SearchDate>,
      date: date == freezed ? _value.date : date as List<SearchDate>,
      evaluated_resource: evaluated_resource == freezed
          ? _value.evaluated_resource
          : evaluated_resource as List<String>,
      identifier: identifier == freezed
          ? _value.identifier
          : identifier as List<SearchToken>,
      measure: measure == freezed ? _value.measure : measure as List<String>,
      patient: patient == freezed ? _value.patient : patient as List<String>,
      period: period == freezed ? _value.period : period as List<SearchDate>,
      reporter:
          reporter == freezed ? _value.reporter : reporter as List<String>,
      status: status == freezed ? _value.status : status as List<SearchToken>,
      subject: subject == freezed ? _value.subject : subject as List<String>,
    ));
  }
}

/// @nodoc
class _$_MeasureReportSearch extends _MeasureReportSearch {
  _$_MeasureReportSearch(
      {this.searchId,
      this.searchLastUpdated,
      this.date,
      this.evaluated_resource,
      this.identifier,
      this.measure,
      this.patient,
      this.period,
      this.reporter,
      this.status,
      this.subject})
      : super._();

  @override
  final List<Id> searchId;
  @override
  final List<SearchDate> searchLastUpdated;
  @override //List<SearchToken> searchTag,
//List<SearchUri> searchProfile,
//List<SearchToken> searchSecurity,
//List<String> searchText,
//List<String> searchContent,
//List<SearchString> searchList,
// List<SearchString> searchHas,
//List<SearchToken> searchType,
  final List<SearchDate> date;
  @override
  final List<String> evaluated_resource;
  @override
  final List<SearchToken> identifier;
  @override
  final List<String> measure;
  @override
  final List<String> patient;
  @override
  final List<SearchDate> period;
  @override
  final List<String> reporter;
  @override
  final List<SearchToken> status;
  @override
  final List<String> subject;

  @override
  String toString() {
    return 'MeasureReportSearch(searchId: $searchId, searchLastUpdated: $searchLastUpdated, date: $date, evaluated_resource: $evaluated_resource, identifier: $identifier, measure: $measure, patient: $patient, period: $period, reporter: $reporter, status: $status, subject: $subject)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other is _MeasureReportSearch &&
            (identical(other.searchId, searchId) ||
                const DeepCollectionEquality()
                    .equals(other.searchId, searchId)) &&
            (identical(other.searchLastUpdated, searchLastUpdated) ||
                const DeepCollectionEquality()
                    .equals(other.searchLastUpdated, searchLastUpdated)) &&
            (identical(other.date, date) ||
                const DeepCollectionEquality().equals(other.date, date)) &&
            (identical(other.evaluated_resource, evaluated_resource) ||
                const DeepCollectionEquality()
                    .equals(other.evaluated_resource, evaluated_resource)) &&
            (identical(other.identifier, identifier) ||
                const DeepCollectionEquality()
                    .equals(other.identifier, identifier)) &&
            (identical(other.measure, measure) ||
                const DeepCollectionEquality()
                    .equals(other.measure, measure)) &&
            (identical(other.patient, patient) ||
                const DeepCollectionEquality()
                    .equals(other.patient, patient)) &&
            (identical(other.period, period) ||
                const DeepCollectionEquality().equals(other.period, period)) &&
            (identical(other.reporter, reporter) ||
                const DeepCollectionEquality()
                    .equals(other.reporter, reporter)) &&
            (identical(other.status, status) ||
                const DeepCollectionEquality().equals(other.status, status)) &&
            (identical(other.subject, subject) ||
                const DeepCollectionEquality().equals(other.subject, subject)));
  }

  @override
  int get hashCode =>
      runtimeType.hashCode ^
      const DeepCollectionEquality().hash(searchId) ^
      const DeepCollectionEquality().hash(searchLastUpdated) ^
      const DeepCollectionEquality().hash(date) ^
      const DeepCollectionEquality().hash(evaluated_resource) ^
      const DeepCollectionEquality().hash(identifier) ^
      const DeepCollectionEquality().hash(measure) ^
      const DeepCollectionEquality().hash(patient) ^
      const DeepCollectionEquality().hash(period) ^
      const DeepCollectionEquality().hash(reporter) ^
      const DeepCollectionEquality().hash(status) ^
      const DeepCollectionEquality().hash(subject);

  @override
  _$MeasureReportSearchCopyWith<_MeasureReportSearch> get copyWith =>
      __$MeasureReportSearchCopyWithImpl<_MeasureReportSearch>(
          this, _$identity);
}

abstract class _MeasureReportSearch extends MeasureReportSearch {
  _MeasureReportSearch._() : super._();
  factory _MeasureReportSearch(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<SearchDate> date,
      List<String> evaluated_resource,
      List<SearchToken> identifier,
      List<String> measure,
      List<String> patient,
      List<SearchDate> period,
      List<String> reporter,
      List<SearchToken> status,
      List<String> subject}) = _$_MeasureReportSearch;

  @override
  List<Id> get searchId;
  @override
  List<SearchDate> get searchLastUpdated;
  @override //List<SearchToken> searchTag,
//List<SearchUri> searchProfile,
//List<SearchToken> searchSecurity,
//List<String> searchText,
//List<String> searchContent,
//List<SearchString> searchList,
// List<SearchString> searchHas,
//List<SearchToken> searchType,
  List<SearchDate> get date;
  @override
  List<String> get evaluated_resource;
  @override
  List<SearchToken> get identifier;
  @override
  List<String> get measure;
  @override
  List<String> get patient;
  @override
  List<SearchDate> get period;
  @override
  List<String> get reporter;
  @override
  List<SearchToken> get status;
  @override
  List<String> get subject;
  @override
  _$MeasureReportSearchCopyWith<_MeasureReportSearch> get copyWith;
}

/// @nodoc
class _$TestReportSearchTearOff {
  const _$TestReportSearchTearOff();

// ignore: unused_element
  _TestReportSearch call(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<SearchToken> identifier,
      List<SearchDate> issued,
      List<SearchUri> participant,
      List<SearchToken> result,
      List<SearchString> tester,
      List<String> testscript}) {
    return _TestReportSearch(
      searchId: searchId,
      searchLastUpdated: searchLastUpdated,
      identifier: identifier,
      issued: issued,
      participant: participant,
      result: result,
      tester: tester,
      testscript: testscript,
    );
  }
}

/// @nodoc
// ignore: unused_element
const $TestReportSearch = _$TestReportSearchTearOff();

/// @nodoc
mixin _$TestReportSearch {
  List<Id> get searchId;
  List<SearchDate> get searchLastUpdated; //List<SearchToken> searchTag,
//List<SearchUri> searchProfile,
//List<SearchToken> searchSecurity,
//List<String> searchText,
//List<String> searchContent,
//List<SearchString> searchList,
// List<SearchString> searchHas,
//List<SearchToken> searchType,
  List<SearchToken> get identifier;
  List<SearchDate> get issued;
  List<SearchUri> get participant;
  List<SearchToken> get result;
  List<SearchString> get tester;
  List<String> get testscript;

  $TestReportSearchCopyWith<TestReportSearch> get copyWith;
}

/// @nodoc
abstract class $TestReportSearchCopyWith<$Res> {
  factory $TestReportSearchCopyWith(
          TestReportSearch value, $Res Function(TestReportSearch) then) =
      _$TestReportSearchCopyWithImpl<$Res>;
  $Res call(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<SearchToken> identifier,
      List<SearchDate> issued,
      List<SearchUri> participant,
      List<SearchToken> result,
      List<SearchString> tester,
      List<String> testscript});
}

/// @nodoc
class _$TestReportSearchCopyWithImpl<$Res>
    implements $TestReportSearchCopyWith<$Res> {
  _$TestReportSearchCopyWithImpl(this._value, this._then);

  final TestReportSearch _value;
  // ignore: unused_field
  final $Res Function(TestReportSearch) _then;

  @override
  $Res call({
    Object searchId = freezed,
    Object searchLastUpdated = freezed,
    Object identifier = freezed,
    Object issued = freezed,
    Object participant = freezed,
    Object result = freezed,
    Object tester = freezed,
    Object testscript = freezed,
  }) {
    return _then(_value.copyWith(
      searchId: searchId == freezed ? _value.searchId : searchId as List<Id>,
      searchLastUpdated: searchLastUpdated == freezed
          ? _value.searchLastUpdated
          : searchLastUpdated as List<SearchDate>,
      identifier: identifier == freezed
          ? _value.identifier
          : identifier as List<SearchToken>,
      issued: issued == freezed ? _value.issued : issued as List<SearchDate>,
      participant: participant == freezed
          ? _value.participant
          : participant as List<SearchUri>,
      result: result == freezed ? _value.result : result as List<SearchToken>,
      tester: tester == freezed ? _value.tester : tester as List<SearchString>,
      testscript: testscript == freezed
          ? _value.testscript
          : testscript as List<String>,
    ));
  }
}

/// @nodoc
abstract class _$TestReportSearchCopyWith<$Res>
    implements $TestReportSearchCopyWith<$Res> {
  factory _$TestReportSearchCopyWith(
          _TestReportSearch value, $Res Function(_TestReportSearch) then) =
      __$TestReportSearchCopyWithImpl<$Res>;
  @override
  $Res call(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<SearchToken> identifier,
      List<SearchDate> issued,
      List<SearchUri> participant,
      List<SearchToken> result,
      List<SearchString> tester,
      List<String> testscript});
}

/// @nodoc
class __$TestReportSearchCopyWithImpl<$Res>
    extends _$TestReportSearchCopyWithImpl<$Res>
    implements _$TestReportSearchCopyWith<$Res> {
  __$TestReportSearchCopyWithImpl(
      _TestReportSearch _value, $Res Function(_TestReportSearch) _then)
      : super(_value, (v) => _then(v as _TestReportSearch));

  @override
  _TestReportSearch get _value => super._value as _TestReportSearch;

  @override
  $Res call({
    Object searchId = freezed,
    Object searchLastUpdated = freezed,
    Object identifier = freezed,
    Object issued = freezed,
    Object participant = freezed,
    Object result = freezed,
    Object tester = freezed,
    Object testscript = freezed,
  }) {
    return _then(_TestReportSearch(
      searchId: searchId == freezed ? _value.searchId : searchId as List<Id>,
      searchLastUpdated: searchLastUpdated == freezed
          ? _value.searchLastUpdated
          : searchLastUpdated as List<SearchDate>,
      identifier: identifier == freezed
          ? _value.identifier
          : identifier as List<SearchToken>,
      issued: issued == freezed ? _value.issued : issued as List<SearchDate>,
      participant: participant == freezed
          ? _value.participant
          : participant as List<SearchUri>,
      result: result == freezed ? _value.result : result as List<SearchToken>,
      tester: tester == freezed ? _value.tester : tester as List<SearchString>,
      testscript: testscript == freezed
          ? _value.testscript
          : testscript as List<String>,
    ));
  }
}

/// @nodoc
class _$_TestReportSearch extends _TestReportSearch {
  _$_TestReportSearch(
      {this.searchId,
      this.searchLastUpdated,
      this.identifier,
      this.issued,
      this.participant,
      this.result,
      this.tester,
      this.testscript})
      : super._();

  @override
  final List<Id> searchId;
  @override
  final List<SearchDate> searchLastUpdated;
  @override //List<SearchToken> searchTag,
//List<SearchUri> searchProfile,
//List<SearchToken> searchSecurity,
//List<String> searchText,
//List<String> searchContent,
//List<SearchString> searchList,
// List<SearchString> searchHas,
//List<SearchToken> searchType,
  final List<SearchToken> identifier;
  @override
  final List<SearchDate> issued;
  @override
  final List<SearchUri> participant;
  @override
  final List<SearchToken> result;
  @override
  final List<SearchString> tester;
  @override
  final List<String> testscript;

  @override
  String toString() {
    return 'TestReportSearch(searchId: $searchId, searchLastUpdated: $searchLastUpdated, identifier: $identifier, issued: $issued, participant: $participant, result: $result, tester: $tester, testscript: $testscript)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other is _TestReportSearch &&
            (identical(other.searchId, searchId) ||
                const DeepCollectionEquality()
                    .equals(other.searchId, searchId)) &&
            (identical(other.searchLastUpdated, searchLastUpdated) ||
                const DeepCollectionEquality()
                    .equals(other.searchLastUpdated, searchLastUpdated)) &&
            (identical(other.identifier, identifier) ||
                const DeepCollectionEquality()
                    .equals(other.identifier, identifier)) &&
            (identical(other.issued, issued) ||
                const DeepCollectionEquality().equals(other.issued, issued)) &&
            (identical(other.participant, participant) ||
                const DeepCollectionEquality()
                    .equals(other.participant, participant)) &&
            (identical(other.result, result) ||
                const DeepCollectionEquality().equals(other.result, result)) &&
            (identical(other.tester, tester) ||
                const DeepCollectionEquality().equals(other.tester, tester)) &&
            (identical(other.testscript, testscript) ||
                const DeepCollectionEquality()
                    .equals(other.testscript, testscript)));
  }

  @override
  int get hashCode =>
      runtimeType.hashCode ^
      const DeepCollectionEquality().hash(searchId) ^
      const DeepCollectionEquality().hash(searchLastUpdated) ^
      const DeepCollectionEquality().hash(identifier) ^
      const DeepCollectionEquality().hash(issued) ^
      const DeepCollectionEquality().hash(participant) ^
      const DeepCollectionEquality().hash(result) ^
      const DeepCollectionEquality().hash(tester) ^
      const DeepCollectionEquality().hash(testscript);

  @override
  _$TestReportSearchCopyWith<_TestReportSearch> get copyWith =>
      __$TestReportSearchCopyWithImpl<_TestReportSearch>(this, _$identity);
}

abstract class _TestReportSearch extends TestReportSearch {
  _TestReportSearch._() : super._();
  factory _TestReportSearch(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<SearchToken> identifier,
      List<SearchDate> issued,
      List<SearchUri> participant,
      List<SearchToken> result,
      List<SearchString> tester,
      List<String> testscript}) = _$_TestReportSearch;

  @override
  List<Id> get searchId;
  @override
  List<SearchDate> get searchLastUpdated;
  @override //List<SearchToken> searchTag,
//List<SearchUri> searchProfile,
//List<SearchToken> searchSecurity,
//List<String> searchText,
//List<String> searchContent,
//List<SearchString> searchList,
// List<SearchString> searchHas,
//List<SearchToken> searchType,
  List<SearchToken> get identifier;
  @override
  List<SearchDate> get issued;
  @override
  List<SearchUri> get participant;
  @override
  List<SearchToken> get result;
  @override
  List<SearchString> get tester;
  @override
  List<String> get testscript;
  @override
  _$TestReportSearchCopyWith<_TestReportSearch> get copyWith;
}

/// @nodoc
class _$TestScriptSearchTearOff {
  const _$TestScriptSearchTearOff();

// ignore: unused_element
  _TestScriptSearch call(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<SearchToken> context,
      List<String> context_quantity,
      List<SearchToken> context_type,
      List<SearchDate> date,
      List<SearchString> description,
      List<SearchToken> identifier,
      List<SearchToken> jurisdiction,
      List<SearchString> name,
      List<SearchString> publisher,
      List<SearchToken> status,
      List<SearchString> testscript_capability,
      List<SearchString> title,
      List<SearchUri> url,
      List<SearchToken> version,
      List<String> context_type_quantity,
      List<String> context_type_value}) {
    return _TestScriptSearch(
      searchId: searchId,
      searchLastUpdated: searchLastUpdated,
      context: context,
      context_quantity: context_quantity,
      context_type: context_type,
      date: date,
      description: description,
      identifier: identifier,
      jurisdiction: jurisdiction,
      name: name,
      publisher: publisher,
      status: status,
      testscript_capability: testscript_capability,
      title: title,
      url: url,
      version: version,
      context_type_quantity: context_type_quantity,
      context_type_value: context_type_value,
    );
  }
}

/// @nodoc
// ignore: unused_element
const $TestScriptSearch = _$TestScriptSearchTearOff();

/// @nodoc
mixin _$TestScriptSearch {
  List<Id> get searchId;
  List<SearchDate> get searchLastUpdated; //List<SearchToken> searchTag,
//List<SearchUri> searchProfile,
//List<SearchToken> searchSecurity,
//List<String> searchText,
//List<String> searchContent,
//List<SearchString> searchList,
// List<SearchString> searchHas,
//List<SearchToken> searchType,
  List<SearchToken> get context;
  List<String> get context_quantity;
  List<SearchToken> get context_type;
  List<SearchDate> get date;
  List<SearchString> get description;
  List<SearchToken> get identifier;
  List<SearchToken> get jurisdiction;
  List<SearchString> get name;
  List<SearchString> get publisher;
  List<SearchToken> get status;
  List<SearchString> get testscript_capability;
  List<SearchString> get title;
  List<SearchUri> get url;
  List<SearchToken> get version;
  List<String> get context_type_quantity;
  List<String> get context_type_value;

  $TestScriptSearchCopyWith<TestScriptSearch> get copyWith;
}

/// @nodoc
abstract class $TestScriptSearchCopyWith<$Res> {
  factory $TestScriptSearchCopyWith(
          TestScriptSearch value, $Res Function(TestScriptSearch) then) =
      _$TestScriptSearchCopyWithImpl<$Res>;
  $Res call(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<SearchToken> context,
      List<String> context_quantity,
      List<SearchToken> context_type,
      List<SearchDate> date,
      List<SearchString> description,
      List<SearchToken> identifier,
      List<SearchToken> jurisdiction,
      List<SearchString> name,
      List<SearchString> publisher,
      List<SearchToken> status,
      List<SearchString> testscript_capability,
      List<SearchString> title,
      List<SearchUri> url,
      List<SearchToken> version,
      List<String> context_type_quantity,
      List<String> context_type_value});
}

/// @nodoc
class _$TestScriptSearchCopyWithImpl<$Res>
    implements $TestScriptSearchCopyWith<$Res> {
  _$TestScriptSearchCopyWithImpl(this._value, this._then);

  final TestScriptSearch _value;
  // ignore: unused_field
  final $Res Function(TestScriptSearch) _then;

  @override
  $Res call({
    Object searchId = freezed,
    Object searchLastUpdated = freezed,
    Object context = freezed,
    Object context_quantity = freezed,
    Object context_type = freezed,
    Object date = freezed,
    Object description = freezed,
    Object identifier = freezed,
    Object jurisdiction = freezed,
    Object name = freezed,
    Object publisher = freezed,
    Object status = freezed,
    Object testscript_capability = freezed,
    Object title = freezed,
    Object url = freezed,
    Object version = freezed,
    Object context_type_quantity = freezed,
    Object context_type_value = freezed,
  }) {
    return _then(_value.copyWith(
      searchId: searchId == freezed ? _value.searchId : searchId as List<Id>,
      searchLastUpdated: searchLastUpdated == freezed
          ? _value.searchLastUpdated
          : searchLastUpdated as List<SearchDate>,
      context:
          context == freezed ? _value.context : context as List<SearchToken>,
      context_quantity: context_quantity == freezed
          ? _value.context_quantity
          : context_quantity as List<String>,
      context_type: context_type == freezed
          ? _value.context_type
          : context_type as List<SearchToken>,
      date: date == freezed ? _value.date : date as List<SearchDate>,
      description: description == freezed
          ? _value.description
          : description as List<SearchString>,
      identifier: identifier == freezed
          ? _value.identifier
          : identifier as List<SearchToken>,
      jurisdiction: jurisdiction == freezed
          ? _value.jurisdiction
          : jurisdiction as List<SearchToken>,
      name: name == freezed ? _value.name : name as List<SearchString>,
      publisher: publisher == freezed
          ? _value.publisher
          : publisher as List<SearchString>,
      status: status == freezed ? _value.status : status as List<SearchToken>,
      testscript_capability: testscript_capability == freezed
          ? _value.testscript_capability
          : testscript_capability as List<SearchString>,
      title: title == freezed ? _value.title : title as List<SearchString>,
      url: url == freezed ? _value.url : url as List<SearchUri>,
      version:
          version == freezed ? _value.version : version as List<SearchToken>,
      context_type_quantity: context_type_quantity == freezed
          ? _value.context_type_quantity
          : context_type_quantity as List<String>,
      context_type_value: context_type_value == freezed
          ? _value.context_type_value
          : context_type_value as List<String>,
    ));
  }
}

/// @nodoc
abstract class _$TestScriptSearchCopyWith<$Res>
    implements $TestScriptSearchCopyWith<$Res> {
  factory _$TestScriptSearchCopyWith(
          _TestScriptSearch value, $Res Function(_TestScriptSearch) then) =
      __$TestScriptSearchCopyWithImpl<$Res>;
  @override
  $Res call(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<SearchToken> context,
      List<String> context_quantity,
      List<SearchToken> context_type,
      List<SearchDate> date,
      List<SearchString> description,
      List<SearchToken> identifier,
      List<SearchToken> jurisdiction,
      List<SearchString> name,
      List<SearchString> publisher,
      List<SearchToken> status,
      List<SearchString> testscript_capability,
      List<SearchString> title,
      List<SearchUri> url,
      List<SearchToken> version,
      List<String> context_type_quantity,
      List<String> context_type_value});
}

/// @nodoc
class __$TestScriptSearchCopyWithImpl<$Res>
    extends _$TestScriptSearchCopyWithImpl<$Res>
    implements _$TestScriptSearchCopyWith<$Res> {
  __$TestScriptSearchCopyWithImpl(
      _TestScriptSearch _value, $Res Function(_TestScriptSearch) _then)
      : super(_value, (v) => _then(v as _TestScriptSearch));

  @override
  _TestScriptSearch get _value => super._value as _TestScriptSearch;

  @override
  $Res call({
    Object searchId = freezed,
    Object searchLastUpdated = freezed,
    Object context = freezed,
    Object context_quantity = freezed,
    Object context_type = freezed,
    Object date = freezed,
    Object description = freezed,
    Object identifier = freezed,
    Object jurisdiction = freezed,
    Object name = freezed,
    Object publisher = freezed,
    Object status = freezed,
    Object testscript_capability = freezed,
    Object title = freezed,
    Object url = freezed,
    Object version = freezed,
    Object context_type_quantity = freezed,
    Object context_type_value = freezed,
  }) {
    return _then(_TestScriptSearch(
      searchId: searchId == freezed ? _value.searchId : searchId as List<Id>,
      searchLastUpdated: searchLastUpdated == freezed
          ? _value.searchLastUpdated
          : searchLastUpdated as List<SearchDate>,
      context:
          context == freezed ? _value.context : context as List<SearchToken>,
      context_quantity: context_quantity == freezed
          ? _value.context_quantity
          : context_quantity as List<String>,
      context_type: context_type == freezed
          ? _value.context_type
          : context_type as List<SearchToken>,
      date: date == freezed ? _value.date : date as List<SearchDate>,
      description: description == freezed
          ? _value.description
          : description as List<SearchString>,
      identifier: identifier == freezed
          ? _value.identifier
          : identifier as List<SearchToken>,
      jurisdiction: jurisdiction == freezed
          ? _value.jurisdiction
          : jurisdiction as List<SearchToken>,
      name: name == freezed ? _value.name : name as List<SearchString>,
      publisher: publisher == freezed
          ? _value.publisher
          : publisher as List<SearchString>,
      status: status == freezed ? _value.status : status as List<SearchToken>,
      testscript_capability: testscript_capability == freezed
          ? _value.testscript_capability
          : testscript_capability as List<SearchString>,
      title: title == freezed ? _value.title : title as List<SearchString>,
      url: url == freezed ? _value.url : url as List<SearchUri>,
      version:
          version == freezed ? _value.version : version as List<SearchToken>,
      context_type_quantity: context_type_quantity == freezed
          ? _value.context_type_quantity
          : context_type_quantity as List<String>,
      context_type_value: context_type_value == freezed
          ? _value.context_type_value
          : context_type_value as List<String>,
    ));
  }
}

/// @nodoc
class _$_TestScriptSearch extends _TestScriptSearch {
  _$_TestScriptSearch(
      {this.searchId,
      this.searchLastUpdated,
      this.context,
      this.context_quantity,
      this.context_type,
      this.date,
      this.description,
      this.identifier,
      this.jurisdiction,
      this.name,
      this.publisher,
      this.status,
      this.testscript_capability,
      this.title,
      this.url,
      this.version,
      this.context_type_quantity,
      this.context_type_value})
      : super._();

  @override
  final List<Id> searchId;
  @override
  final List<SearchDate> searchLastUpdated;
  @override //List<SearchToken> searchTag,
//List<SearchUri> searchProfile,
//List<SearchToken> searchSecurity,
//List<String> searchText,
//List<String> searchContent,
//List<SearchString> searchList,
// List<SearchString> searchHas,
//List<SearchToken> searchType,
  final List<SearchToken> context;
  @override
  final List<String> context_quantity;
  @override
  final List<SearchToken> context_type;
  @override
  final List<SearchDate> date;
  @override
  final List<SearchString> description;
  @override
  final List<SearchToken> identifier;
  @override
  final List<SearchToken> jurisdiction;
  @override
  final List<SearchString> name;
  @override
  final List<SearchString> publisher;
  @override
  final List<SearchToken> status;
  @override
  final List<SearchString> testscript_capability;
  @override
  final List<SearchString> title;
  @override
  final List<SearchUri> url;
  @override
  final List<SearchToken> version;
  @override
  final List<String> context_type_quantity;
  @override
  final List<String> context_type_value;

  @override
  String toString() {
    return 'TestScriptSearch(searchId: $searchId, searchLastUpdated: $searchLastUpdated, context: $context, context_quantity: $context_quantity, context_type: $context_type, date: $date, description: $description, identifier: $identifier, jurisdiction: $jurisdiction, name: $name, publisher: $publisher, status: $status, testscript_capability: $testscript_capability, title: $title, url: $url, version: $version, context_type_quantity: $context_type_quantity, context_type_value: $context_type_value)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other is _TestScriptSearch &&
            (identical(other.searchId, searchId) ||
                const DeepCollectionEquality()
                    .equals(other.searchId, searchId)) &&
            (identical(other.searchLastUpdated, searchLastUpdated) ||
                const DeepCollectionEquality()
                    .equals(other.searchLastUpdated, searchLastUpdated)) &&
            (identical(other.context, context) ||
                const DeepCollectionEquality()
                    .equals(other.context, context)) &&
            (identical(other.context_quantity, context_quantity) ||
                const DeepCollectionEquality()
                    .equals(other.context_quantity, context_quantity)) &&
            (identical(other.context_type, context_type) ||
                const DeepCollectionEquality()
                    .equals(other.context_type, context_type)) &&
            (identical(other.date, date) ||
                const DeepCollectionEquality().equals(other.date, date)) &&
            (identical(other.description, description) ||
                const DeepCollectionEquality()
                    .equals(other.description, description)) &&
            (identical(other.identifier, identifier) ||
                const DeepCollectionEquality()
                    .equals(other.identifier, identifier)) &&
            (identical(other.jurisdiction, jurisdiction) ||
                const DeepCollectionEquality()
                    .equals(other.jurisdiction, jurisdiction)) &&
            (identical(other.name, name) ||
                const DeepCollectionEquality().equals(other.name, name)) &&
            (identical(other.publisher, publisher) ||
                const DeepCollectionEquality()
                    .equals(other.publisher, publisher)) &&
            (identical(other.status, status) ||
                const DeepCollectionEquality().equals(other.status, status)) &&
            (identical(other.testscript_capability, testscript_capability) ||
                const DeepCollectionEquality().equals(
                    other.testscript_capability, testscript_capability)) &&
            (identical(other.title, title) ||
                const DeepCollectionEquality().equals(other.title, title)) &&
            (identical(other.url, url) ||
                const DeepCollectionEquality().equals(other.url, url)) &&
            (identical(other.version, version) ||
                const DeepCollectionEquality()
                    .equals(other.version, version)) &&
            (identical(other.context_type_quantity, context_type_quantity) ||
                const DeepCollectionEquality().equals(
                    other.context_type_quantity, context_type_quantity)) &&
            (identical(other.context_type_value, context_type_value) ||
                const DeepCollectionEquality()
                    .equals(other.context_type_value, context_type_value)));
  }

  @override
  int get hashCode =>
      runtimeType.hashCode ^
      const DeepCollectionEquality().hash(searchId) ^
      const DeepCollectionEquality().hash(searchLastUpdated) ^
      const DeepCollectionEquality().hash(context) ^
      const DeepCollectionEquality().hash(context_quantity) ^
      const DeepCollectionEquality().hash(context_type) ^
      const DeepCollectionEquality().hash(date) ^
      const DeepCollectionEquality().hash(description) ^
      const DeepCollectionEquality().hash(identifier) ^
      const DeepCollectionEquality().hash(jurisdiction) ^
      const DeepCollectionEquality().hash(name) ^
      const DeepCollectionEquality().hash(publisher) ^
      const DeepCollectionEquality().hash(status) ^
      const DeepCollectionEquality().hash(testscript_capability) ^
      const DeepCollectionEquality().hash(title) ^
      const DeepCollectionEquality().hash(url) ^
      const DeepCollectionEquality().hash(version) ^
      const DeepCollectionEquality().hash(context_type_quantity) ^
      const DeepCollectionEquality().hash(context_type_value);

  @override
  _$TestScriptSearchCopyWith<_TestScriptSearch> get copyWith =>
      __$TestScriptSearchCopyWithImpl<_TestScriptSearch>(this, _$identity);
}

abstract class _TestScriptSearch extends TestScriptSearch {
  _TestScriptSearch._() : super._();
  factory _TestScriptSearch(
      {List<Id> searchId,
      List<SearchDate> searchLastUpdated,
      List<SearchToken> context,
      List<String> context_quantity,
      List<SearchToken> context_type,
      List<SearchDate> date,
      List<SearchString> description,
      List<SearchToken> identifier,
      List<SearchToken> jurisdiction,
      List<SearchString> name,
      List<SearchString> publisher,
      List<SearchToken> status,
      List<SearchString> testscript_capability,
      List<SearchString> title,
      List<SearchUri> url,
      List<SearchToken> version,
      List<String> context_type_quantity,
      List<String> context_type_value}) = _$_TestScriptSearch;

  @override
  List<Id> get searchId;
  @override
  List<SearchDate> get searchLastUpdated;
  @override //List<SearchToken> searchTag,
//List<SearchUri> searchProfile,
//List<SearchToken> searchSecurity,
//List<String> searchText,
//List<String> searchContent,
//List<SearchString> searchList,
// List<SearchString> searchHas,
//List<SearchToken> searchType,
  List<SearchToken> get context;
  @override
  List<String> get context_quantity;
  @override
  List<SearchToken> get context_type;
  @override
  List<SearchDate> get date;
  @override
  List<SearchString> get description;
  @override
  List<SearchToken> get identifier;
  @override
  List<SearchToken> get jurisdiction;
  @override
  List<SearchString> get name;
  @override
  List<SearchString> get publisher;
  @override
  List<SearchToken> get status;
  @override
  List<SearchString> get testscript_capability;
  @override
  List<SearchString> get title;
  @override
  List<SearchUri> get url;
  @override
  List<SearchToken> get version;
  @override
  List<String> get context_type_quantity;
  @override
  List<String> get context_type_value;
  @override
  _$TestScriptSearchCopyWith<_TestScriptSearch> get copyWith;
}
